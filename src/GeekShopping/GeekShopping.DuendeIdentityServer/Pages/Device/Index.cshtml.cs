...
using GeekShopping.DuendeIdentityServer.Pages.Consent;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.RazorPages;
using Microsoft.Extensions.Options;

namespace GeekShopping.DuendeIdentityServer.Pages.Device
{
    [SecurityHeaders]
    [Authorize]
    public class Index : PageModel
    {
        ...
        {
            DeviceFlowAuthorizationRequest? request = await _interaction
                .GetAuthorizationContextAsync(Input.UserCode ?? throw new ArgumentNullException(nameof(Input.UserCode)));
            ...
            else if(Input.Button == "yes")
            {
                ...
                if(Input.ScopesConsented.Any())
                {
                    ...
                    if(ConsentOptions.EnableOfflineAccess.Equals(false))
                    {
                        scopes = scopes.Where((x) => !x.Equals(Duende.IdentityServer.IdentityServerConstants.StandardScopes.OfflineAccess));
                    }
                    ...
                    IEnumerable<string> denied = request
                        .ValidatedResources
                        .ParsedScopes
                        .Select((s) => s.ParsedName)
            ...
            {
                // Communicates an outcome of consent back to IdentityServer:
                await _interaction.HandleRequestAsync(Input.UserCode, grantedConsent);

                // Indicate that's it ok to redirect back to the authorization endpoint:
        ...
        private ViewModel CreateConsentViewModel(DeviceFlowAuthorizationRequest request)
        {
            ViewModel vm = new ViewModel
            {
                ...
                IdentityScopes = request.ValidatedResources
                    .Resources
                    .IdentityResources
                    .Select((x) => CreateScopeViewModel(x, (Input is null)|| (Input.ScopesConsented.Contains(x.Name))))
                    .ToArray()
            };
            ...
            if(DeviceOptions.EnableOfflineAccess && request.ValidatedResources.Resources.OfflineAccess)
            {
                apiScopes.Add(
                    GetOfflineAccessScope(
                        Input is null ||
                        Input.ScopesConsented.Contains(Duende.IdentityServer.IdentityServerConstants.StandardScopes.OfflineAccess)
                    )
                );
            }
