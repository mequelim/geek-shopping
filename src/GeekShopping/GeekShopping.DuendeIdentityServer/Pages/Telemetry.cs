...
namespace GeekShopping.DuendeIdentityServer.Pages
{
    ...
    public static class Telemetry
    {
        ...
        /// </summary>
        private static readonly string ServiceName = typeof(Telemetry).Assembly.GetName().Name!;
        ...
        public static class Metrics
        {
            ...
            /// <summary>
            /// Helper method to increase <see cref="Counters.Consent"/> counter.
            /// The scopes are expanded and called one by one to not cause a combinatory explosion of scopes.
            /// </summary>
            ...
            public static void ConsentDenied(string clientId, IEnumerable<string> scopes)
            {
                ...
                foreach(string scope in scopes)
                {
                    ConsentCounter.Add(
                        delta: 1,
                        tag1: new KeyValuePair<string, object?>(Tags.Client, clientId),
                        tag2: new KeyValuePair<string, object?>(Tags.Scope, scope),
                        tag3: new KeyValuePair<string, object?>(Tags.Consent, TagValues.Denied));
                }
            }
            ...
            public static void UserLogin(string? clientId, string idp)
            {
                UserLoginCounter.Add(
                    delta: 1,
                    tag1: new KeyValuePair<string, object?>(Tags.Client, clientId),
                    tag2: new KeyValuePair<string, object?>(Tags.Idp, idp));
            }
            ...
            public static void UserLoginFailure(string? clientId, string idp, string error)
            {
                UserLoginCounter.Add(
                    ...
                    tag1: new KeyValuePair<string, object?>(Tags.Client, clientId),
                    tag2:new KeyValuePair<string, object?>(Tags.Idp, idp),
                    tag3: new KeyValuePair<string, object?>(Tags.Error, error)
                );
            }
            ...
            {
                UserLogoutCounter.Add(
                    delta: 1,
                    tag: new KeyValuePair<string, object?>(Tags.Idp, idp)
                );
            }
        }
    }
