...
namespace GeekShopping.CartAPI.RabbitMQ.Sender
{
    /// <summary>
    ...
    public class RabbitMqMessageSender(IConfiguration configuration) : IRabbitMqMessageSender
    {
        ...
        /// The object should typically match the expected data contract, such as <see cref="CheckoutHeaderDto"/>.
        /// </param>
        ...
        {
            string json = JsonSerializer.Serialize((CheckoutHeaderDto)message, JsonOptions);
        ...
        /// </summary>
        /// <param name="baseMessage">The message to be sent, encapsulated in a <see cref="BaseMessage"/> object.</param>
        /// <param name="queueName">The name of the RabbitMQ queue where the message will be sent.</param>
        /// <returns>A task that represents the asynchronous operation of sending the message.</returns>
        /// <exception cref="InvalidOperationException">Thrown when a RabbitMQ connection cannot be established.</exception>
        /// <exception cref="ArgumentNullException">Thrown when the <paramref name="baseMessage"/> or <paramref name="queueName"/> parameter is null.</exception>
